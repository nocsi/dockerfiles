# roster.nocsi.org/swagger  

[`roster.nocsi.org/swagger`][1] is a [docker][2] image that bundles the following:  
...
docker run -it --rm \
	...
	-p 8080:8080 \
	roster.nocsi.org/swagger:1.0.0-beta.2 validate /data/swagger.yaml  
````
...
docker run -it --rm \
	...
	-e "PGID=$(id -g)" -e "PUID=$(id -u)" \
	-p 8080:8080 \
	roster.nocsi.org/swagger:1.0.0-beta.2 bundle --dereference --outfile /output/swagger.yaml /data/swagger.yaml  
````
...
[1]: https://hub.docker.com/r/roster.nocsi.org/swagger/   
[2]: https://docker.com 
